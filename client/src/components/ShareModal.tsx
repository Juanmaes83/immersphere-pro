import { useEffect, useRef, useState } from 'react';
import QRCode from 'qrcode';
import { propertyShareUrl } from '@/utils/slugify';

interface Props {
  propertyId: string;
  title: string;
  description?: string;
  imageUrl?: string;
  primaryColor?: string;
  // NOTE: whatsappNumber removed — contact button lives in the property aside, not here
  onClose: () => void;
}

export default function ShareModal({
  propertyId, title, description, imageUrl, primaryColor = '#7C3AED',
  onClose,
}: Props): JSX.Element {
  const canonicalUrl = propertyShareUrl(propertyId, title);
  const [copied, setCopied] = useState(false);
  const qrCanvasRef = useRef<HTMLCanvasElement>(null);

  // Generate QR on mount
  useEffect(() => {
    if (!qrCanvasRef.current) return;
    void QRCode.toCanvas(qrCanvasRef.current, canonicalUrl, {
      width: 140, margin: 2,
      color: { dark: '#0f172a', light: '#ffffff' },
    });
  }, [canonicalUrl]);

  // Close on backdrop click or Escape
  useEffect(() => {
    function onKey(e: KeyboardEvent) { if (e.key === 'Escape') onClose(); }
    window.addEventListener('keydown', onKey);
    return () => window.removeEventListener('keydown', onKey);
  }, [onClose]);

  function handleCopy() {
    void navigator.clipboard.writeText(canonicalUrl).then(() => {
      setCopied(true);
      setTimeout(() => setCopied(false), 2000);
    });
  }

  const encodedText = encodeURIComponent(`🏠 Mira este piso que he encontrado: ${title}\n¿Qué te parece?\n${canonicalUrl}`);
  const encodedDesc = encodeURIComponent(description ?? title);

  const socials = [
    {
      label: 'WhatsApp',
      icon: (
        <svg viewBox="0 0 24 24" fill="currentColor" className="h-5 w-5">
          <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z"/>
        </svg>
      ),
      href: `https://wa.me/?text=${encodedText}`,
      bg: '#25D366',
      text: 'white',
    },
    {
      label: 'X / Twitter',
      icon: (
        <svg viewBox="0 0 24 24" fill="currentColor" className="h-5 w-5">
          <path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-4.714-6.231-5.401 6.231H2.748l7.73-8.835L1.254 2.25H8.08l4.261 5.635zm-1.161 17.52h1.833L7.084 4.126H5.117z"/>
        </svg>
      ),
      href: `https://twitter.com/intent/tweet?text=${encodedText}`,
      bg: '#000000',
      text: 'white',
    },
    {
      label: 'Email',
      icon: (
        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2} className="h-5 w-5">
          <path d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" strokeLinecap="round" strokeLinejoin="round"/>
        </svg>
      ),
      href: `mailto:?subject=${encodeURIComponent(title)}&body=${encodedDesc}%0A%0A${encodeURIComponent(canonicalUrl)}`,
      bg: '#475569',
      text: 'white',
    },
  ];

  return (
    /* Backdrop */
    <div
      className="fixed inset-0 z-50 flex items-end justify-center bg-black/50 backdrop-blur-sm sm:items-center"
      onClick={onClose}
    >
      {/* Panel */}
      <div
        className="w-full max-w-md rounded-t-[2rem] bg-white p-6 shadow-2xl sm:rounded-[2rem]"
        onClick={e => e.stopPropagation()}
      >
        {/* Header */}
        <div className="mb-5 flex items-center justify-between">
          <h2 className="text-lg font-black text-slate-900">Compartir propiedad</h2>
          <button
            type="button"
            onClick={onClose}
            className="flex h-8 w-8 items-center justify-center rounded-full bg-slate-100 text-slate-500 hover:bg-slate-200"
          >
            <svg className="h-4 w-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2.5}>
              <path d="M6 18L18 6M6 6l12 12" strokeLinecap="round"/>
            </svg>
          </button>
        </div>

        {/* Property preview */}
        {imageUrl && (
          <div className="mb-4 overflow-hidden rounded-2xl">
            <img src={imageUrl} alt={title} className="h-32 w-full object-cover" />
          </div>
        )}
        <p className="mb-1 font-black text-slate-900">{title}</p>

        {/* Canonical URL box */}
        <div className="mt-3 flex items-center gap-2 rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3">
          <svg className="h-4 w-4 shrink-0 text-slate-400" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2}>
            <path d="M13.828 10.172a4 4 0 00-5.656 0l-4 4a4 4 0 105.656 5.656l1.102-1.101m-.758-4.899a4 4 0 005.656 0l4-4a4 4 0 00-5.656-5.656l-1.1 1.1" strokeLinecap="round" strokeLinejoin="round"/>
          </svg>
          <span className="flex-1 truncate text-xs font-semibold text-slate-600">{canonicalUrl}</span>
          <button
            type="button"
            onClick={handleCopy}
            className="shrink-0 rounded-xl px-3 py-1.5 text-xs font-black transition"
            style={{ backgroundColor: copied ? '#10b981' : primaryColor, color: 'white' }}
          >
            {copied ? '✓ Copiado' : 'Copiar'}
          </button>
        </div>

        {/* Social share buttons */}
        <div className="mt-4 grid grid-cols-3 gap-2">
          {socials.map(s => (
            <a
              key={s.label}
              href={s.href}
              target="_blank"
              rel="noopener noreferrer"
              className="flex flex-col items-center gap-1.5 rounded-2xl py-3 text-xs font-bold transition hover:opacity-90"
              style={{ backgroundColor: s.bg, color: s.text }}
            >
              {s.icon}
              {s.label}
            </a>
          ))}
        </div>

        {/* QR code */}
        <div className="mt-4 flex items-center gap-4 rounded-2xl border border-slate-200 bg-slate-50 p-4">
          <canvas ref={qrCanvasRef} className="rounded-lg" />
          <div>
            <p className="text-sm font-black text-slate-800">Código QR</p>
            <p className="mt-1 text-xs text-slate-500 leading-5">
              Escanea para abrir el tour directamente en el móvil
            </p>
          </div>
        </div>

        {/* Native share (mobile) */}
        {typeof navigator !== 'undefined' && typeof navigator.share === 'function' && (
          <button
            type="button"
            onClick={() => {
              void navigator.share({ title, text: `🏠 Mira este piso que he encontrado: ${title}\n¿Qué te parece?`, url: canonicalUrl }).catch(() => {});
            }}
            className="mt-3 w-full rounded-2xl border border-slate-200 py-3 text-sm font-black text-slate-700 hover:bg-slate-50"
          >
            Más opciones para compartir…
          </button>
        )}
      </div>
    </div>
  );
}
